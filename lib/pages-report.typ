...
#let page-title-report(
  ...
  if doc.school.orientation != none or doc.school.url != none {
    [#safe-link(url: doc.school.url, name: doc.school.orientation)]
  }
  ...
  if doc.authors.first().name != none {
    [*#enumerating-emails(names: doc.authors.map(a => a.name), emails: doc.authors.map(a => a.email))*\ ]
  }
  ...
  table(
    ...
    align: (left, right),
    [
      #if doc.logos.tp_bottomleft != none {
        [#doc.logos.tp_bottomleft]
      }
    ],
    [
      #if doc.logos.tp_bottomright != none {
        [#doc.logos.tp_bottomright]
      }
    ],
  )
}
